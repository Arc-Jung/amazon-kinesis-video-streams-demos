...
VOID addCanaryMetadataToFrameData(PFrame pFrame) {
    ...
    putInt64((PINT64) (pFrame->frameData + SIZEOF(UINT64) + SIZEOF(UINT32) + SIZEOF(UINT32)), COMPUTE_CRC32(pFrame->frameData, pFrame->size));
}
...
CleanUp:
    ...
    if (STATUS_FAILED(retStatus)) {
        defaultLogPrint(LOG_LEVEL_ERROR, (PCHAR)"", (PCHAR)"Failed with status 0x%08x\n", retStatus);
    }
    ...
    defaultLogPrint(LOG_LEVEL_INFO, (PCHAR)"", (PCHAR)"CleanUp Done\n");
